    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            if(lhs->m_size!=rhs->m_size)
                return 0;
            for(i=0;i<lhs->m_size;i++)
            {
               index= lept_find_object_index(rhs,lhs->m[i].k,lhs->m[i].klen);
               if(index==LEPT_KEY_NOT_EXIST)return 0;
               if(!lept_is_equal(&lhs->m[i].v,&rhs->m[index].v))
                   return 0;
            }
            return 1;
        default:
...
void lept_copy(lept_value* dst, const lept_value* src) {
    ...
    switch (src->type) {
        ...
        case LEPT_OBJECT:
            /* \todo */
            lept_set_object(dst,src->m_size);
            for(i=0;i<src->m_size;i++)
            {
                lept_value *val= lept_set_object_value(dst,src->m[i].k,src->m[i].klen);
                lept_copy(val,&src->m[i].v);
            }
...
void lept_shrink_array(lept_value* v) {
    ...
    if (v->capacity > v->size) {
        v->capacity = v->size;
        v->e = (lept_value*)realloc(v->e, v->capacity * sizeof(lept_member));
    }
}
...
lept_value* lept_insert_array_element(lept_value* v, size_t index){
    ...
    if(v->size==v->capacity)
        lept_reserve_array(v,v->capacity==0?1:(v->capacity<<1));
    memcpy(&v->e[index + 1], &v->e[index], (v->size - index) * sizeof(lept_value));
    lept_init(&v->e[index]);
    v->size++;
    return &v->e[index];
}
void lept_erase_array_element(lept_value* v, size_t index, size_t count){
    assert(v!=NULL&&v->type==LEPT_ARRAY&&v->size>=index+count);
    size_t i;
    for(i=index;i<index+count;i++)
    lept_free(&v->e[i]);
    memcpy(v->e+index,v->e+index+count,(v->size-index-count)*sizeof(lept_value));
...
lept_value * lept_set_object_value(lept_value*v,const char*key,size_t klen)
{
    ...
    if(v->m_size==v->m_capacity){
        lept_reserve_object(v,v->m_capacity==0?1:v->m_capacity<<1);
    }
    i=v->m_size;
...
{
    assert(v!=NULL&&v->type==LEPT_OBJECT&&index<v->m_size);
    free(v->m[index].k);
    lept_free(&v->m[index].v);
    memcpy(v->m+index,v->m+index+1,(v->m_size-index-1)*sizeof(lept_member));
    v->m[--v->m_size].k=NULL;
    v->m[v->m_size].klen=0;
    lept_init(&v->m[v->m_size].v);
}
...
void lept_reserve_object(lept_value* v, size_t capacity){
    ...
    if (v->m_capacity < capacity) {
        v->m_capacity = capacity;
        v->m = (lept_member*)realloc(v->m, capacity * sizeof(lept_member));
    }
}
void lept_shrink_object(lept_value*v){
    ...
    if(v->m_capacity>v->m_size)
    {
        ...
        v->m=(lept_member*) realloc(v->m,v->m_capacity*sizeof(lept_member));
    }
}
